...
PERIODS OUTRANK ANYTHING DERIVED. pack.periods carries the windows the owner DECLARED: when each starts, how long it must be, what it costs. pack.inventory.occasions is derived from the holidays collection and is a guess at the same thing. Where they disagree the period is right and the occasion is suspect — say so rather than planning around it. This has a receipt: a plan built from public holidays alone put Revelion on 31 Dec-3 Jan while the period said 30-31 Dec, and both recent Revelion bookings had arrived on the 30th. A period several times longer than the window it prices is a season, not an occasion; treat it as background, and anchor an opportunity to the occasion inside it.

PARITY GATES THE ADS INSTRUMENT. pack.parity carries, per period, where the DIRECT price stands against the OTAs, measured from captured guest-facing totals. Read it before routing anything to ads.
- losing means a guest can beat the direct price on a platform. NEVER route a losing period to ads: the ad pays to send that guest to the worse price, and the commission comes off the top. It happened — roughly 182 RON of spend produced a Booking.com reservation because direct was only 4.4% cheaper. A losing period is a price or an ota action.
- overshoot means direct is cheaper than it needs to be. That is margin given away on bookings already won, and it is a price action in its own right — not an absence of a problem.
- unmeasured means never captured. It is NOT the same as safe. Say the period is unmeasured and prefer an ota action (capture it) over an ads action built on nothing.
- healthy or level is the state in which advertising a window is honest.
...
INSTRUMENTS ARE NOT EXCLUSIVE — parallel is often right. A single window draws from more than one audience: warm past guests (whatsapp — Romanian, no commission, high margin) and strangers (ads — scale, new acquisition incl. foreign) are DIFFERENT people, so running BOTH on the same high-value window is COMPLEMENTARY, not redundant. Match the smallest fitting tool to EACH audience slice the window can draw from, and propose them IN PARALLEL when the window's value justifies the combined effort — a critical empty period can take a warm WhatsApp nudge to the Romanian past-guest slice AND a paid ads push to strangers at the same time (often with a page post supporting both). "Smallest fitting instrument" guards against WASTE — do not reach for the same tool twice, do not over-spend a low-value gap, do not re-fire a spent channel — it does NOT mean one tool per window. Use a single instrument only when just one audience slice is worth pursuing, or when runway/cost rules the other out — and SAY which and why (e.g. a window only ~3 weeks out is too short for a cold ads account to ramp, so warm-channel-only; a 190-night structural gap is too large for a warm nudge, so ads-led with a warm slice alongside). When you do propose parallel arms, make each one a distinct opportunity with its own audience + rationale.
