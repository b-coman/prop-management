...
export async function buildCopywriterPack(brief: CampaignBrief, opts?: { asOf?: Date; ownerName?: string }): Promise<CopywriterPack> {
  ...
  // The owner quotes round figures: in campaign A he changed every price to the 50 lei below
  // (2,287 -> 2,250, 2,647 -> 2,600) and left everything else as written. The site price stays in
  // the fact so nobody loses track of what a guest booking on the site would pay.
  const ownerRound = (lei: number) => Math.floor(lei / 50) * 50;
  ...
  const guests = wantIds.map(gid => {
    ...
    if (partyPrice && brief.stay) groundedFacts.push({ key: 'priceForParty', value: { guests: Number(lastBk.numberOfGuests), totalLei: ownerRound(partyPrice), sitePriceLei: partyPrice, checkIn: brief.stay.checkIn, checkOut: brief.stay.checkOut }, source: 'quoteStay (the site price), rounded down to 50 lei as the owner quotes it' });
  ...
  return {
    ...
    voiceRules: {
      ...
      register: 'Address the PERSON you are writing to as the owner does in that thread: tu if he uses tu with them, dumneavoastra if he is formal, and polite voi/dumneavoastra for a first contact. When you mean the whole family or group, voi is natural ("pentru voi", "va astept") - the owner writes "iti scriu... pentru voi" on purpose, it is not a mistake. What must not happen is switching the person themself between tu and dumneavoastra in one message.',
      ...
      master: 'If campaign.masterMessage is set, it IS the message: the owner wrote or approved it, and its framing stays. Keep his greeting style, his sentences, their order, his closing and his PS, in his words. Personalise it for THIS guest, and only this way: (1) right after the greeting, add one or two sentences that continue your conversation with them (cite thread:<ts>) or recall their stay; (2) use their name and the tu/voi that fits them; (3) if the guest has a `priceForParty` fact, put its totalLei (already rounded the way the owner quotes) and their party size where the master gives an example or no price, as "cam 2.250 lei" or "2.250 lei" (dot for thousands, tag priceForParty); never quote sitePriceLei - without it (a lead, an unknown size) keep the master as it is; (4) drop or adjust a master line that is wrong for them (a kids line for a group without kids, a self-introduction for someone you talk to often); (5) add at most one short line when the conversation calls for it (their dog, the other family they mentioned, the direct-booking line for an OTA-only guest). Do NOT rephrase his sentences and do NOT add descriptions, adjectives or selling points he did not write (never describe the house - bedrooms, yard - to people who stayed there). campaign.generalAngle and campaign.occasion.point are background for you, never wording to use.',
