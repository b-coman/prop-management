...
export async function buildCopywriterPack(brief: CampaignBrief, opts?: { asOf?: Date; ownerName?: string }): Promise<CopywriterPack> {
  ...
  return {
    ...
    voiceRules: {
      ...
      master: 'If campaign.masterMessage is set, the owner approved its CONTENT: the dates, the prices, the offer, the PS lines. Carry all of that, and add no campaign claim it does not make. Its WORDING is his example, not a template: write THIS guest\'s message as your next message in your conversation with them, in your own words, and use his phrasing only where it fits naturally. Keep the owner\'s greeting style from the master (e.g. "Hello!...", "Salut", "Buna") and add the name; he varies greetings on purpose. What must not be copied is the rest of the opening: never open the body with the master\'s first sentence for a guest you have actually talked to. PRICE: if the guest has a `priceForParty` fact, quote THAT price for their party size (written with a dot for thousands) and tag priceForParty; only without it (a lead, an unknown party size) keep the example price from the master and offer the exact price for their group. When there is no masterMessage, write from generalAngle as below.',
