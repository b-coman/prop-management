 ...
 * Performs the Firestore query server-side to improve security
 */
export async function GET(request: NextRequest) {
  try {
    ...
    // Implementation directly from the previous getUnavailableDatesForProperty function
    const unavailableDates: string[] = []; // Return as ISO strings for consistent serialization
    
    if (!db) {
      console.error("[API] ❌ CRITICAL ERROR: Firestore Client SDK (db) is not initialized.");
      return NextResponse.json(
        {
          error: 'Database connection error - Firestore client not initialized',
    ...
    console.log("[API] ✅ Firestore DB client is initialized, continuing with availability check");
    ...
    // Execute all query batches in parallel
