...
export function PricingTestPanel({ propertyId }: { propertyId: string }) {
  ...
  const loadTestScript = (scriptType: string) => {
    ...
    // Files in public/scripts/ folder are served from /scripts/ URL path
    // Add timestamp to avoid caching issues
    const timestamp = new Date().getTime();
    
    // Try direct paths first (in public folder root)
    let scriptPath = `/browser-test-api.js?t=${timestamp}`;
    if (scriptType === 'ui') {
      scriptPath = `/browser-test-ui.js?t=${timestamp}`;
      console.log('Loading UI test script:', scriptPath);
    ...
    console.log(`Full URL would be: ${window.location.origin}${scriptPath}`);
  ...
  const openPropertyInNewTab = () => {
    ...
    // Most property IDs in the system are already slugs, but in case they're not
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              <Button 
                ...
                onClick={() => {
                  const script = document.createElement('script');
                  script.src = `/scripts/test-script.js?t=${new Date().getTime()}`;
                  document.body.appendChild(script);
                  console.log('Test script added to page');
                }}
              >
                Test Script
              </Button>
              ...
              <Button 
                ...
                onClick={() => {
                  const script = document.createElement('script');
                  script.src = `/test-direct.js?t=${new Date().getTime()}`;
                  document.body.appendChild(script);
                  console.log('Direct test script added to page');
                }}
              >
                Test Direct
              </Button>
              ...
              <Button 
                ...
                onClick={() => {
                  const script = document.createElement('script');
                  script.src = `/browser-test-ui.js?t=${new Date().getTime()}`;
                  document.body.appendChild(script);
                  console.log('Loading browser-test-ui.js directly from public folder');
                }}
              >
                Load UI Script
              </Button>
              ...
              <div className="flex flex-col items-center justify-center py-8 text-center">
                ...
                <div className="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-md max-w-md">
                  <p className="text-sm text-amber-800">
                    <strong>May 13-14, 2025 Issue:</strong> This tool can help identify if consecutive blocked dates like May 13-14, 2025 are being correctly handled for checkout.
                  </p>
                </div>
              </div>
            </Card>
