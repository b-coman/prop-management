...
export default function ClientSeasonalPricingTable({ propertyId }: ClientSeasonalPricingTableProps) {
  ...
  useEffect(() => {
    const loadSeasons = async () => {
      ...
      try {
        console.log('Loading seasonal pricing for property:', propertyId);
        ...
        try {
          const seasonsCollection = collection(db, 'seasonalPricing');
          const q = query(
            seasonsCollection,
          ...
          console.log('Query complete, documents:', querySnapshot.size);

          const fetchedSeasons: SeasonalPricing[] = [];
          querySnapshot.forEach((doc) => {
            ...
            fetchedSeasons.push({
              ...
              name: data.name,
              seasonType: data.seasonType || 'standard',
          ...
          console.log('Processed seasons:', fetchedSeasons.length);
          setSeasons(fetchedSeasons);
        } catch (firestoreErr) {
          console.error('Firestore query error:', firestoreErr);
          setError('Failed to load seasonal pricing data from database');
          setSeasons([]);
        }
      } catch (err) {
        ...
        // Always set seasons to empty array so UI doesn't stay in loading state
        setSeasons([]);
      } finally {
        setLoading(false);
      }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const seasonRef = doc(db, 'seasonalPricing', deleteId);
  ...
  if (seasons.length === 0) {
    return (
      <div className="py-8 text-center">
        ...
        <Button 
          onClick={() => router.push(`/manage-pricing/seasons/new?propertyId=${propertyId}`)}
        >
          Create First Season
        </Button>
      </div>
              ...
              <TableCell className="text-right">
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem
                      onClick={() => router.push(`/manage-pricing/seasons/edit/${season.id}`)}
                    >
                      Edit
                    </DropdownMenuItem>
                    <DropdownMenuItem
