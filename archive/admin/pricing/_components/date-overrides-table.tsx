...
import { DateOverride } from '@/lib/pricing/price-calculation';
import { getDateOverrides, updateDateOverride, deleteDateOverride } from '../actions';

interface DateOverridesTableProps {
  propertyId: string;
}

export default function DateOverridesTable({ propertyId }: DateOverridesTableProps) {
  ...
  useEffect(() => {
    const loadOverrides = async () => {
      ...
      try {
        const result = await getDateOverrides(propertyId);
        
        if (result.success) {
          setOverrides(result.overrides);
        } else {
          setError(result.error || 'Failed to load date overrides');
        }
      } catch (err) {
  ...
  const handleAvailableToggle = async (id: string, currentValue: boolean) => {
    try {
      const result = await updateDateOverride(id, { available: !currentValue });
      ...
      if (result.success) {
        ...
        setError(result.error || 'Failed to update date override');
      }
    } catch (err) {
  ...
  const handleFlatRateToggle = async (id: string, currentValue: boolean) => {
    try {
      const result = await updateDateOverride(id, { flatRate: !currentValue });
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const result = await deleteDateOverride(deleteId);
      ...
      if (result.success) {
        ...
        setError(result.error || 'Failed to delete date override');
      }
    } catch (err) {
              ...
              <TableCell className="text-right">
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem
                      onClick={() => router.push(`/admin/pricing/date-overrides/edit/${override.id}`)}
                    >
                      Edit
                    </DropdownMenuItem>
                    <DropdownMenuItem
